...
from tomachess.registry import GenericRegistry, HasType, TournamentRegistry, TeamTournamentRegistry
from tomachess.store.abstract_store import AbstractStore

T = TypeVar("T", bound=Entity)
U = TypeVar("U", bound=HasType)
...
class JsonStore(AbstractStore):
    ...
    def _load_models_registry(self, folder: str, collection: str, registry: Type[GenericRegistry[U]]) -> list[U]:
        path = self._get_file_path(folder, collection)
        data = self._load_json(path)
        return [registry.require(item["type"])(**item) for item in data.values()]
    ...
    def load_tournaments(self, collection: str) -> list[TournamentBase]:
        return self._load_models_registry("tournaments", collection, TournamentRegistry)
    ...
    def load_team_tournaments(self, collection: str) -> list[TeamTournamentBase]:
        return self._load_models_registry("team_tournaments", collection, TeamTournamentRegistry)
